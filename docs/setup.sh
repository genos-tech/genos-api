...
curl --header "Content-Type: application/json" \
  ...
  http://192.168.10.13:8890/api/v2/user/signup/

# Run API without JWT
curl -H "Content-Type: application/json" \
  --request POST \
  --data '{"st_chat_group_name":"Chat Group1", "bl_personal":"True"}' \
  http://192.168.10.13:8890/api/v2/chatGroup/create/
...
# Login and get JWT
ACCESS=$(curl --header "Content-Type: application/json" --request POST --data '{"username":"asd3","password":"xyz"}' http://192.168.10.13:8890/api/v2/user/login/ | jq -r '.access')
...
curl -H "Authorization: Bearer ${ACCESS}" \
  --request GET \
  http://192.168.10.13:8890/api/v2/test/

# Get all users
curl -H "Authorization: Bearer ${ACCESS}" \
  --request GET \
  http://192.168.10.13:8890/api/v2/user/listAllUsers/

# Get all chat groups
curl -H "Authorization: Bearer ${ACCESS}" \
  --request GET \
  http://192.168.10.13:8890/api/v2/chatGroup/myGroups/
