...
class PersonalNoteMasterView(AuthenticatedAPIView):
    ...
    def put(self, request):
        ...
        if "body" in update_data:
            ...
            # Suppress mention notifications for PERSONAL notes only.
            #
            # A personal note has no implicit access, so an @mention in an
            # unshared one pinged the recipient about something that 403s
            # — noise with no recourse. Those are filtered to the people
            # the note actually reaches, and the rest come back as
            # `nonMemberMentions` so the author is offered the share.
            #
            # TEAM notes must keep notifying EVERYONE mentioned, including
            # people not yet in the folder. A team note is a shared
            # surface: being mentioned into one is how you find out it
            # exists, and the author gets the "add them" prompt to close
            # the access gap. Filtering them by folder reach — which an
            # earlier version of this did — meant a mention into a private
            # folder reached nobody and the recipient never learned of it.
            if not is_team_folder(note.folder_id):
                reachable = reachable_mentions(
                    full_mentioned, personal_note_id=note.note_id
                )
                newly_mentioned_user_ids = [
                    u for u in newly_mentioned_user_ids if u in reachable
                ]
