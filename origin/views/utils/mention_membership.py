...
def non_member_mentions(
    ...
    *,
    ...
    """Mentioned users who can't reach the surface.

    Returns `[{"userId", "userName", "scopeKind"}]`, empty when every
    mention can already see it — which is the common case, so callers
    can attach the field unconditionally and the client can branch on
    emptiness alone.

    Pass exactly one scope. A surface with NO scope (an unfiled personal
    note) resolves to "nobody is missing": sharing that is a per-note
    grant the note-sharing dialog already owns, not a membership gap.
    """
    ...
    if project_id is not None:
        ...
        is_team_folder = PersonalNoteFolder.objects.filter(
