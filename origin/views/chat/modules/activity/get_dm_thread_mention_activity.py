...
def get(user_id: str, team_id: str, my_all_dm_ids, n_days_ago: datetime):
    ...
    _dm_me_mentioned_thread_messages = DMThreadMessages.objects.filter(
        dm__team=team_id,
        ts_sent_at__gte=n_days_ago,
    ).filter(
        Q(dm__in=list(set([row["chat_id"] for row in dm_raw_me_mentions])))
        & Q(thread_message_id__in=list(set([row["message_id"] for row in dm_raw_me_mentions])))
    )

    dm_me_mentioned_thread_messages = []
    for message in _dm_me_mentioned_thread_messages:
        ...
        dm_me_mentioned_thread_messages.append(
            {
                ...
                "messageUniqueKey": f"{message.dm.dm_id}-{message.thread_message_id}",
    ...
    return dm_me_mentioned_thread_messages
